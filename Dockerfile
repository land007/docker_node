...
RUN apt-get --allow-releaseinfo-change update && apt-get install -y python ca-certificates && apt-get clean
...
#ENV SHIPPABLE_NODE_VERSION=v16.15.0
ENV SHIPPABLE_NODE_VERSION=v14.20.0
...
RUN echo 'export SHIPPABLE_NODE_VERSION=v14.20.0' >> /etc/profile && \
...
RUN dos2unix /root/.nvm/versions/node/v14.20.0/lib/node_modules/supervisor/lib/cli-wrapper.js
...
#> docker buildx build --platform linux/amd64,linux/arm64/v8,linux/arm/v7 -t land007/node:14.20 --push .
#> docker buildx build --platform linux/amd64,linux/arm/v7 -t land007/node:14.20 --push .
